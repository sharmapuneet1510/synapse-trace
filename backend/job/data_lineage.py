...
# ── core modules ─────────────────────────────────────────────────────────────
import sys
_JOB_DIR = os.path.dirname(__file__)                           # backend/job/
_ROOT = os.path.dirname(os.path.dirname(_JOB_DIR))            # project root
for _p in (_JOB_DIR, _ROOT):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from trace_core.indexers.repo_indexer import RepoIndexer, Index
from trace_core.tracing.trace_context import TraceContext
from trace_core.tracing.field_trace_engine import FieldTraceEngine
from trace_core.tracing.branch_trace_engine import BranchTraceEngine
from trace_core.graph.nx_graph_builder import NxGraphBuilder
from trace_core.exporters.trace_result import TraceResult
from trace_core.exporters.html_exporter import HtmlExporter
from trace_core.explain.trace_summarizer import TraceSummarizer
...
class Graph:
    ...
    def _run_prompt(
        ...
        try:
            from backend.app.prompts import prompt_registry, jinja_engine
    ...
    def to_neo4j(self, output_dir: Optional[str] = None) -> str:
        """Export the NetworkX graph as a Neo4j Cypher script and save to disk.

        Writes ``<output_dir>/<KEY>_graph.cypher`` with MERGE statements for
        every node and relationship in the NetworkX graph.  The file can be
        executed in Neo4j Browser (``CALL apoc.cypher.runFile(...)``) or piped
        to ``cypher-shell``.

        Returns the absolute path of the written ``.cypher`` file.
        To get the in-memory payload instead, call ``self.result.to_neo4j()``.
        """
        ...
        payload  = Neo4jExporter().export(nx_graph)
        stmts    = payload.get("cypher_statements", [])

        lines = [
            f"// Neo4j Cypher — lineage graph for field: {self._key}",
            f"// Nodes: {nx_graph.number_of_nodes()}  Edges: {nx_graph.number_of_edges()}",
            f"// Generated by Synapse-Trace  ({len(stmts)} statements)",
            "",
        ]
        lines.extend(
            stmt + ";" if not stmt.rstrip().endswith(";") else stmt
            for stmt in stmts
        )
        ...
        with open(path, "w", encoding="utf-8") as fh:
            fh.write(cypher)
        _logger.info(f"Neo4j Cypher export saved: {path}")
        return path
...
class Trace:
    ...
    def to_graph(self, output_dir: Optional[str] = None) -> "Graph":
        """Return a Graph wrapper and immediately save ``{fieldname}_graph.html``.

        The saved HTML is a standalone fullscreen vis.js network showing every
        node and edge from the NetworkX ``MultiDiGraph`` attached to the result.
        It is written to *output_dir* (default: ``backend/output/html/``).

        Returns
        -------
        Graph
            Wrapper object exposing ``to_html()``, ``to_md()``, ``to_json()``,
            ``to_neo4j()`` and ``extend()``.
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        for field in fields:
            ...
            graph = trace.to_graph()     # wraps TraceResult; NX graph rendered in to_html()
            graphs.append(graph)
